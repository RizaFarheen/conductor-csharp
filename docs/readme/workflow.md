...
This section will dive deeper into creating and executing Conductor workflows using C# SDK.
...
Conductor lets you create the workflows using either C# or JSON as the configuration.

Using C# as code to define and execute workflows lets you build extremely powerful, dynamic workflows and run them on Conductor.
...
See [dynamic_workflow.cs] for a fully functional example.

### Kitchen-Sink Workflow

For a more complex workflow example with all the supported features, see [kitchensink.cs].
...
>[!note]
>See [workflow_ops.cs] for a fully working application that demonstrates working with the workflow executions and sending signals to the workflow to manage its state.
